...
def main():
    parser = argparse.ArgumentParser(description="Process Vector arguments")

    parser.add_argument("--num_chars", "-nc", default=256, help="defines the number of "
                                                                "characters to use for this run of the"
                                                                " vector generator")
    parser.add_argument("--vector_name", "-vn", default="generated_tv", help="defines the name of the"
                                                                            " testvector output")
    parser.add_argument("--max_match_length", "-ml", default=15, help="defines the max match length")
    args = parser.parse_args()

    num_chars = args.num_chars
    vector_name = args.vector_name
    max_match_length = args.max_match_length

    random_string = generate_string(num_chars)
    compressed_string = compress_string(random_string, max_match_length)
    write_out_binary_data(compressed_string, random_string, vector_name)


def write_out_binary_data(compressed_data, decompressed_data, vector_name):
    """
...
def compress_string(string_to_compress, max_match_length):
    """
    ...
    for i in range (0, len(string_to_compress)):
        ...
        if key in hash_table and i < len(string_to_compress) - 2:
            ...
            length = get_length_of_match(string_to_compress[i:last_match_idx], temp_raw_index, max_match_length)
...
def get_length_of_match(string_to_match, initial_raw_index, max_match_length):
    """
    ...
    for char in string_to_match:
        if char == full_char_array[current_index]:
            match_length += 1
            current_index += 1
        else:
...
def generate_string(num_chars):
    ...
    return generate_alphanumeric_random_string(num_chars)


def generate_alphanumeric_random_string(num_chars):
    """
